...
const roles = [
    { id: 0, role: 'Filter by Status'},
    { id: 1, role: 'Accepted' },
    { id: 2, role: 'Rejected' },
]

export function GlobalFilterRequestStatus({
    ...
    useEffect(() => {
        if (selectedRole.role == 'Filter by Status'){
        ...
        <SearchContainer>
            <Listbox value={selectedRole} onChange={setSelectedRole}>
                <Listbox.Button tw="flex w-60 p-4 h-10 border-2 border-gray-200 rounded-lg text-left text-sm text-gray-600 justify-between items-center truncate">
                    <span placeholder="Status">{selectedRole.role}</span>
